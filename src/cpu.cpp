...
//
// 硬件自适应: 通过在运行时动态检测 CPU 的 ISA 特性，ncnn 能够为用户的硬件自动选择最优化的代码路径，而不需要用户为每种 CPU 单独编译一个版本。
// 跨平台抽象: 通过 CpuSet 等类，它将底层操作系统中关于线程调度和亲和性设置的复杂、不统一的 API 封装成了简洁、统一的接口。
// 功耗与性能的平衡: 提供了对大小核的精细控制能力，让开发者可以根据应用场景（前台高负载 vs 后台低功耗）来动态调整 ncnn 的资源使用策略。
